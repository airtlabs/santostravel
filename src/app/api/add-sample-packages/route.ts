...
export async function POST() {
    try {
        console.log('Starting bulk package insertion...')

        const supabase = createAdminClient()
        const insertedPackages = []
        const errors = []

        // Insert packages one by one to handle any individual errors
        for (let i = 0; i < samplePackages.length; i++) {
            const packageData = samplePackages[i]

            try {
                console.log(`Inserting package ${i + 1}/${samplePackages.length}: ${packageData.title}`)
                ...
                if (error) {
                    console.error(`Error inserting package "${packageData.title}":`, error)
                    errors.push({
                        package: packageData.title,
                        error: error.message
                    })
                } else {
                    console.log(`Successfully inserted: ${packageData.title}`)
                    insertedPackages.push(data)
                }
            } catch (err) {
                console.error(`Unexpected error inserting package "${packageData.title}":`, err)
                errors.push({
                    package: packageData.title,
        ...
        const summary = {
            ...
            successful: insertedPackages.length,
            failed: errors.length,
            categories: {
                india: insertedPackages.filter(p => p.category === 'india').length,
                world: insertedPackages.filter(p => p.category === 'world').length,
                specialty: insertedPackages.filter(p => p.category === 'specialty').length,
                customized: insertedPackages.filter(p => p.category === 'customized').length,
                corporate: insertedPackages.filter(p => p.category === 'corporate').length
            }
        }

        console.log('Bulk insertion completed:', summary)

        return NextResponse.json({
            status: 'success',
            message: `Successfully inserted ${insertedPackages.length} out of ${samplePackages.length} packages`,
            summary,
            insertedPackages: insertedPackages.map(p => ({ id: p.id, title: p.title, category: p.category })),
            errors: errors.length > 0 ? errors : undefined,
            timestamp: new Date().toISOString()
        })

    } catch (error) {
        console.error('Unexpected error during bulk insertion:', error)
        return NextResponse.json(
            {
                status: 'error',
                message: 'Failed to insert sample packages',
...
export async function GET() {
    return NextResponse.json({
        status: 'info',
        message: 'Use POST method to insert sample packages',
        packageCount: samplePackages.length,
        categories: {
            india: samplePackages.filter(p => p.category === 'india').length,
            world: samplePackages.filter(p => p.category === 'world').length,
            specialty: samplePackages.filter(p => p.category === 'specialty').length,
            customized: samplePackages.filter(p => p.category === 'customized').length,
            corporate: samplePackages.filter(p => p.category === 'corporate').length
        }
    })
}
